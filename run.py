...
def main(yolo):
    ...
    while True:
        ...
        if (selectingObject):
            ...
            # Change to your own parameters/换成自己的目标参数和相机参数
            #Lenovo camera
            ixx0=int((box[0][0])+(box[0][2])/2)
            iyy0=int((box[0][1])+(box[0][3])/2)
            '''
            D0 = 1258.775/h # meter,qianhou
            x_cen = 320
            xd0 = ixx0 - x_cen
            # geo similarity
            X0 =xd0/h*1.75 # meter,zuoyou
            '''
            ...
            # Change to your own parameters/换成自己的目标参数和相机参数
            '''
            D= 1258.775/ht # meter,qianhou
            x_cen = 320
            xd = ix - x_cen
            # geo similarity
            X = xd/ht*1.75 # meter,zuoyou
            '''
            ...
            center = (int((boundingbox[0])+(boundingbox[2])/2),int((boundingbox[1])+(boundingbox[3])/2))
            cv2.circle(frame,(center), 1, (0, 255, 255), 5)
