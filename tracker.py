...
def get_APCE(gaumap):
    ...
    max_F = np.max(gaumap)
    min_F = np.min(gaumap)
    apce = np.square(max_F-min_F) / (np.sum(np.square(gaumap[:,:]-min_F)))
    return apce
# 实部图像
def real(img):
    return img[:, :, 0]
...
class KCFTracker:
    ...
    def update(self, image):
        ...
        # 尺度不变时检测峰值结果
        loc, peak_value, APCE = self.detect(self._tmpl, self.getFeatures(image, 0, 1.0))
        ...
        print(APCE)
        # 使用当前的检测框来训练样本参数,if APCE can be transfered
